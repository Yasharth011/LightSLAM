...
inline void CalcBicubicWeights(float t, float &wm1, float &w0, float &w1,
                               float &w2) {
...
XFeat::XFeat(const std::string &modelFile) {
  ...
  ortEnv_ = std::unique_ptr<Ort::Env>(
      new Ort::Env(ORT_LOGGING_LEVEL_WARNING, "XFeat"));
...
void XFeat::DetectAndCompute(const cv::Mat &img,
                             std::vector<cv::KeyPoint> &keys, cv::Mat &descs,
                             int maxCorners) {
  // check image size
  ...
  if (img.rows < H_ || img.cols < W_) {
    std::cerr << "Image size mismatch!" << img.rows << ", " << img.cols
              << std::endl;
    return;
  }
  ...
  inputTensors.emplace_back(OnnxHelper::CreateTensor<float>(
      inputInfos_[0].shape, fimg.ptr<float>(), W_ * H_, true));
  ...
  auto outputTensors = ortSession_->Run(
      Ort::RunOptions{nullptr}, inputNames_.data(), inputTensors.data(), 1,
  ...
  // the keypoint score tensor [1, H/8, W/8, 65], we drop the last channel(dust
  // bin), and convert to [H, W] image
  cv::Mat scoreImg(H_, W_, CV_32F);
  auto *scoreImgPtr = scoreImg.ptr<float>();
  FlattenScore(kptScorePtr, scoreImgPtr);

  // apply nms, only keep the points with score > 0.05 and is the local maxima
  // in a 5x5 window
  ...
  std::sort(scoredPoints_.begin(), scoredPoints_.end(),
  ...
  for (const auto &pt : scoredPoints_) {
    if (pt.x <= minEdgeX || pt.x >= maxEdgeX || pt.y <= minEdgeY ||
        pt.y >= maxEdgeY) {
      continue;
    }
...
void XFeat::Nms(const cv::Mat &scores, float scoreThresh, int kernelSize,
...
void XFeat::InterpDescriptor(const float *descMat, float *descriptor, float ptx,
                             float pty) {
  int x0 = cvFloor(ptx);
  int y0 = cvFloor(pty);
  int xm1 = x0 - 1;
  ...
  double sum = 0;
  for (int i = 0; i < 64; ++i) {
    float v_m1 = wxm1 * desc_xm1_ym1[i] + wx0 * desc_x0_ym1[i] +
                 wx1 * desc_x1_ym1[i] + wx2 * desc_x2_ym1[i];
    float v_0 = wxm1 * desc_xm1_y0[i] + wx0 * desc_x0_y0[i] +
                wx1 * desc_x1_y0[i] + wx2 * desc_x2_y0[i];
    float v_1 = wxm1 * desc_xm1_y1[i] + wx0 * desc_x0_y1[i] +
                wx1 * desc_x1_y1[i] + wx2 * desc_x2_y1[i];
    float v_2 = wxm1 * desc_xm1_y2[i] + wx0 * desc_x0_y2[i] +
                wx1 * desc_x1_y2[i] + wx2 * desc_x2_y2[i];
