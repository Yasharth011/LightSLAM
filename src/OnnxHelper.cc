...
void OnnxHelper::GetModelInfo(Ort::Session &session,
  ...
  for (size_t i = 0; i < numInputNodes; ++i) {
    ...
    inputInfos.emplace_back(
        TensorInfo{name, inShape, inputTypeInfo.GetONNXType()});
  }
  ...
  for (size_t i = 0; i < numOutputNodes; ++i) {
    ...
    outputInfos.emplace_back(
        TensorInfo{name, outShape, outputTypeInfo.GetONNXType()});
  }
}

void OnnxHelper::PrintModelInfo(const std::vector<TensorInfo> &inputInfos,
...
void OnnxHelper::Str2Ort(const std::string &modelFilePath,
